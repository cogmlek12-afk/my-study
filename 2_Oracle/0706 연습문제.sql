...
-- 1. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
        SELECT EMP_NAME, SUBSTR (EMP_NO, 1,2) "생년" , SUBSTR (EMP_NO, 3,2 )"생월" 
...
-- 6. EMPLOYEE 테이블에서 사원명, 급여 조회 (단, 급여는 '\9,000,000' 형식으로 표시)
      SELECT EMP_NAME 직원명, TO_CHAR(SALARY, 'L9,999,999') SALARY 
      FROM EMPLOYEE;
...
--          나이는 주민번호에서 출력해서 날짜데이터로 변환한 다음 계산)
  SELECT EMP_NAME 직원명, DEPT_CODE 부서코드, CONCAT(SUBSTR(EMP_NO,1,2)  ||'년', SUBSTR(EMP_NO,3,2) || '월') ||
  CONCAT(SUBSTR( EMP_NO,5,2) ,'일') "생년월일", EXTRACT(YEAR FROM SYSDATE) - 
...
--     주민번호 앞자리와 뒷자리의 합 조회
        SELECT EMP_NAME, EMP_ID,SUBSTR(EMP_NO,1,6) "주민번호앞자리" ,SUBSTR(EMP_NO,8,7) "뒷자리", 
...
COUNT(DECODE (EXTRACT(YEAR FROM HIRE_DATE), 2002,1)) ,
COUNT(DECODE (EXTRACT(YEAR FROM HIRE_DATE), 2003,1)) ,
