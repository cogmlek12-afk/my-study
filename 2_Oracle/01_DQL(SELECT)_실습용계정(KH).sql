...
/* 
  ...
  /*
      ...
      */
      ...
      -- 부서코드, 직급 코드를 중복 제거한 상태로 조회 (부서별 직급 현황)
    ...
    */ -- => DISTINCT 는 한번만 사용가능
      ...
      /* 
        ...
        * 비교 연산자 : 동등비교 / 대소비교
           ...
           */
           ...
           -- 직원 중 부서코드가 D1이 아닌 직원들의 정보조회 (이름, 급여, 부서코드)
            SELECT EMP_NAME, SALARY, DEPT_CODE
           FROM EMPLOYEE
           WHERE DEPT_CODE != 'D1';
           ...
           SELECT EMP_NAME AS "직원이름" ,  DEPT_CODE " 부서코드" , SALARY 급여, SALARY * 12  AS 연봉
           FROM EMPLOYEE
      ...
      -- 1. 급여가 300만원 이상인 직원들의 직원명, 급여, 입사일, 연봉 조회
         SELECT EMP_NAME AS "직원이름" ,  SALARY 급여,HIRE_DATE 입사일, SALARY * 12  AS 연봉
           FROM EMPLOYEE
           WHERE SALARY >= 3000000;
           
           
      -- 2. 연봉이 5000만원 이상인 직원들의 직원명, 급여, 연봉, 부서코드 조회
         SELECT EMP_NAME AS "직원이름" ,  SALARY 급여, SALARY * 12  AS 연봉, DEPT_CODE 부서코드
           FROM EMPLOYEE
           WHERE SALARY * 12 >= 50000000;
           
           
      -- 3. 직급코드가 J3이 아닌 직원들의 직원번호, 직원명, 직급코드, 퇴사여부 조회
       SELECT EMP_ID "직원번호" , EMP_NAME "직원이름" , DEPT_CODE "직원코드" , ENT_DATE "퇴사여부"
           FROM EMPLOYEE
           WHERE DEPT_CODE != 'J3';
    ...
    % : 0글자 이상
    ex) 비교대상컬럼 LIKE 'A%' : 'A'로 "시작"되는 것을 조회
          비교대상컬럼 LIKE '%A' : 'A'로 "끝"나는 것을 조회
          비교대상컬럼 LIKE '%문자%' : '문자'가 "포함"되는 것을 조회 (키워드 검색) 
    
    _  : 1글자 
   ex) 비교대상컬럼 LIKE '_문자' : '문자'앞에 무조건 한 글자가 있는 경우 조회
        비교대상컬럼 LIKE'__문자' : '문자'앞에 무조건 두 글자가 있는 경우 조회
  ...
  -- * 직원이름에 '하'가 포함된 이름, 연락처 조회
      SELECT EMP_NAME, PHONE
      FROM EMPLOYEE
      WHERE EMP_NAME LIKE '%하%';
      
 -- * 직원의 연락처 정보에서 3번째 자리가 1인 직원 정보 조회 (이름,연락처)
       SELECT EMP_NAME, PHONE
      FROM EMPLOYEE
      WHERE PHONE LIKE '__1%';
      
-- * 직원 이메일 정보에서 4번째 자리가 _인 직원정보 조회 ( 직원번호, 이름, 이메일)
...
/* 
   IS NULL / IS NOT NULL
    : 컬럼 값에 NULL이 있을경우 NULL 값을 비교할 때 사용하는 연산자
    
    대상컬럼 IS NULL => 컬럽의 값이 NULL인 행을 선택(조회)
    대상컬럼 IS NOT NULL => 컬럽의 값이 NULL이 아닌 행만 선택(조회)
    */
 ...
 --  보너스 (BONUS)를 받지 않는 직원 조회 => NULL 값인 직원만 조회
    SELECT EMP_NAME, BONUS
    FROM EMPLOYEE
    WHERE BONUS IS NULL;
    
-- 사수가 없는 직원 조회 ( 직원번호,이름,사수번호)
    SELECT EMP_ID,EMP_NAME,MANAGER_ID
    FROM EMPLOYEE
    WHERE MANAGER_ID IS NULL;
    
    -- 부서배치는 안받았지만 보너스를 받고있는 직원 조회( 직원명, 보너스, 부서코드)
    ...
    2 순위 : 연결 연산자 : || 
    3 순위 : 비교 연산자 : > < >= <= = != <>
    4 순위 : IS NULL / IS NOT NULL / LIKE '패턴' / IN
    5 순위 : BETWEEN A AND B
    6 순위 : NOT
    7 순위 : AND
    8 순위 : OR
    */
    ...
    --직급코드가 J7이거나 J2인 직원들 중 급여가 200만원 이상인 직원 조회 (모든정보)
      ...
      FROM 테이블명
      WHERE 조건
      ORDER BY 정렬기준이 되는 컬럼 정렬 방식 NULL에 대한 옵션
      ...
      * 정렬 방식 
        ...
        -DESC : 내림차순 정렬
     * NULL에 대한 옵션
         - NULLS FIRST : 정렬 하고자 하는 컬럼의 값이 NULL인 경우 해당 데이터를 맨 앞에 위치 (DESC인 경우 기본값)
         - NULLS LAST : 정렬하고자 하는 컬럼의 값이 NULL인 경우 해당 데이터를 맨 뒤에 위치 (ASC인 경우 기본값)
      */
      
    -- 모든 직원의 직원명, 연봉조회 (내림차순정렬)
